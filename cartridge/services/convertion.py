class Convertion:
    ...
    def get_content(self):
        ...
        content += "\tstatic const glyph_t glyphs[94] = {\n"
        for ascii in range(32, 128):
            pixels_bytes = self.get_pixels_bitmap_glyph(ascii)
            print(f"{ascii}:")
            content += self.get_proportional_glyph(pixels_bytes)
        content += "\t};\n\n"

        content += "\tfont->type = type;\n"
        content += "\tfont->bitmap = font_bitmap;\n"
        content += "\tfont->global = &global;\n"
        content += "\tfont->glyphs = glyphs;\n"
        content += "}\n"
    ...
    def get_proportional_glyph(self, pixel_bytes: bytes) -> str:
        glyph_list: list[str] = self.get_glyph_list(pixel_bytes)
        min_x = None
        max_x = None
        min_y = None
